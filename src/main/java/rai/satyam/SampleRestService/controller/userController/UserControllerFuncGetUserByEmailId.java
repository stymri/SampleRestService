...
public class UserControllerFuncGetUserByEmailId extends UserControllerFuncBase {
	...
	public void startFunction() {
		try {
			...
			if(isValidRepo() && isValidEmail() && isUserExists()) {
	...
	private boolean isUserExists() {
		boolean v_bReturn = false;
		Optional<User> v_objUser;
		try {
			v_objUser = super.g_objUserRepository.findById(this.g_strEmail);
			if(!v_objUser.isPresent()) {
				this.g_Response.setMessage("User not be Exists");
			}else {
				this.g_objUser = v_objUser.get();
				v_bReturn = true;
			}
		}catch(Exception v_exException) {
			v_exException.printStackTrace();
